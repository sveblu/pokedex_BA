...
def make_datasets(
    ...
    # Same augmentations as MobileNetV2 / B0 version
    ...
    if cache_to_disk:
        cache_root = Path.home() / ".tfdata_cache" / "tiny-imagenet-200-efficientnet-b2-aug"
...
def build_model(num_classes: int) -> tuple[keras.Model, keras.Model]:
    ...
    model = keras.Model(inp, out, name="efficientnetb2_tiny_imagenet_aug")
    return model, base
...
def main():
    ap = argparse.ArgumentParser(
        description="EfficientNetB2 on Tiny-ImageNet-200 with Keras augmentations",
    )
    ap.add_argument("--data_root", required=True, help="tiny-imagenet-200 root")
    ap.add_argument("--run_name", default="tinyimagenet_efficientnet_b2_aug")

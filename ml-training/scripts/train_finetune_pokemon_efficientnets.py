...
def default_size_for_variant(variant: str) -> tuple[int, int]:
    if variant == "b0":
        return (224, 224)
    if variant == "b1":
        return (240, 240)
...
def build_ds(root, img_size=(224, 224), batch=32):
    ...
    # EfficientNet: use preprocess_input like in pretraining
    def norm(x, y):
        x = tf.cast(x, tf.float32)      # 0–255 float
        x = preprocess_input(x)         # EfficientNet normalization
        return x, y
...
def unfreeze_tail(model, n_layers):
    unfrozen = 0
    # skip last layer (new Pokémon head)
...
def main():
    ap = argparse.ArgumentParser(
        description="Finetune Tiny-ImageNet-pretrained EfficientNet (b0/b1/b2) on Pokémon dataset (NO AUGS)"
    )
    ...
    ap.add_argument(
        ...
        required=True,
        choices=["b0", "b1", "b2"],
        help="EfficientNet variant that was used during Tiny-ImageNet pretraining",
    )

    ap.add_argument(
        "--pretrain_run",
        required=True,
        help="Name of Tiny-ImageNet run under runs/ (e.g. tinyimagenet_efficientnet_b0_v1)",
    )
    ap.add_argument(
        "--pretrain_ckpt",
        default="best.keras",
    ...
    ap.add_argument(
        ...
        nargs=2,
        default=None,
        help="Image size (H W). If omitted, will use variant default "
    ...
    if args.image_size is None:
        img_size = default_size_for_variant(args.variant)
    else:

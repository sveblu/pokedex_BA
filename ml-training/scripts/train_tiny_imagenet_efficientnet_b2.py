...
# -------------------------------------------------------------------
# Decode / resize with EfficientNet preprocessing
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Dataset construction (no augmentation, optional cache)
# -------------------------------------------------------------------
...
def make_datasets(
    ...
    if cache_to_disk:
        cache_root = Path.home() / ".tfdata_cache" / "tiny-imagenet-200-efficientnet-b2"
...
def build_model(num_classes: int) -> tuple[keras.Model, keras.Model]:
    base = keras.applications.EfficientNetB2(
        input_shape=(260, 260, 3),   # native B2 resolution
        include_top=False,
        weights="imagenet",
    )
    inp = keras.Input(shape=(260, 260, 3))
    x = base(inp, training=False)   # BN in inference mode during warm-up
    x = keras.layers.GlobalAveragePooling2D()(x)
    x = keras.layers.Dropout(0.3)(x)
    out = keras.layers.Dense(num_classes, activation="softmax")(x)
    model = keras.Model(inp, out, name="efficientnetb2_tiny_imagenet")
    return model, base
...
def main():
    ap = argparse.ArgumentParser(
        description="EfficientNetB2 on Tiny-ImageNet-200 (no augmentation)",
    )
    ap.add_argument("--data_root", required=True, help="tiny-imagenet-200 root")
    ap.add_argument("--run_name", default="tinyimagenet_efficientnet_b2")
    ap.add_argument("--batch_size", type=int, default=32)
    ap.add_argument("--image_size", type=int, nargs=2, default=(260, 260))

    ap.add_argument("--epochs_warmup", type=int, default=5)
    ap.add_argument("--epochs_finetune", type=int, default=30)
    ap.add_argument("--unfreeze_last", type=int, default=200)

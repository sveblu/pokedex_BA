...
def _decode_and_resize(img_bytes: tf.Tensor, image_size: tuple[int, int]) -> tf.Tensor:
    ...
    img = preprocess_input(img)  # EfficientNet expects 0–255 float then this
    return img
...
def make_datasets(
    ...
    if cache_to_disk:
        cache_root = Path.home() / ".tfdata_cache" / "tiny-imagenet-200-efficientnet-b1"
...
def build_model(num_classes: int, image_size: tuple[int, int]) -> tuple[keras.Model, keras.Model]:
    ...
    model = keras.Model(inp, out, name="efficientnetb1_tiny_imagenet")
    return model, base
...
def main():
    ap = argparse.ArgumentParser(
        description="EfficientNetB1 on Tiny-ImageNet-200 (no augmentation)",
    )
    ap.add_argument("--data_root", required=True, help="tiny-imagenet-200 root")
    ap.add_argument("--run_name", default="tinyimagenet_efficientnet_b1")
    ap.add_argument("--batch_size", type=int, default=32)
    # B1 canonical size 240x240; override if you want 224x224 for comparability
    ap.add_argument("--image_size", type=int, nargs=2, default=(240, 240))
    ...
    model, base = build_model(num_classes, image_size=image_size)
